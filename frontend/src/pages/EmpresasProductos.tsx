...
export default function EmpresasProductos() {
  ...
  // Función para cargar datos específicamente del embudo Empresas y Productos
  const fetchOpportunities = useCallback(async () => {
    if (isInitialLoad) setIsInitialLoad(false);
    setLoading(true);
    setError(null);
    
    console.log('🔄 Iniciando carga de oportunidades Empresas y Productos...');
    
    try {
      const res = await api.getEmpresasProductosOpportunities({ useCache: true });
      ...
      if (res.success && res.data) {
        ...
        setPipeline(res.data.pipeline?.name || "Empresas y Productos");
        ...
        setError(res.error || "Error al cargar oportunidades del embudo Empresas y Productos");
        setOpps([]);
      }
    } catch (err) {
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              {pipeline ? `Pipeline: ${pipeline}` : 'Gestiona tus oportunidades de empresas y productos'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <p className="text-xl font-semibold text-blue-600">{filteredOpps.length}</p>
            </div>
            <Button
              onClick={() => navigate('/empresas-productos/create')}
                   ...
                   <div className="p-2 space-y-2 max-h-[calc(100vh-300px)] overflow-y-auto">
                     {groupedByStage[stage]?.map((opp, index) => (
                       <motion.div
                         ...
                         className="group"
                         onClick={() => navigate(`/empresas-productos/${opp.id}`)}
                       >
                           ...
                           <CardContent className="p-3">
                             ...
                             <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                               <Button
                                 ...
                                 onClick={(e) => {
                                   e.stopPropagation();
                                   navigate(`/empresas-productos/${opp.id}`);
                                 }}
                               >
